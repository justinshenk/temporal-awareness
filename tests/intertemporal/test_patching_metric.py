...
from src.intertemporal.analysis.patching import PatchingMetric
...
class _MockChoice:
    """Mock BinaryChoice for testing."""
    def __init__(self, choice_idx: int, chosen_label: str):
    ...
    def alternative_idx(self) -> int:
        return 1 - self._choice_idx if self._choice_idx != -1 else -1

    @property
    def choice_logprob(self) -> float:
        return -0.223  # log(0.8)

    @property
    def alternative_logprob(self) -> float:
        return -1.609  # log(0.2)
...
def _make_pref_item(sample_idx, short_label, long_label, choice_term, prompt="prompt"):
    ...
    # Determine choice index and chosen label based on choice_term
    if choice_term == "short_term":
        choice_idx = 0
        chosen_label = short_label
    else:
        choice_idx = 1
        chosen_label = long_label

    response = f"I select: {chosen_label}"
    mock_choice = _MockChoice(choice_idx=choice_idx, chosen_label=chosen_label)
...
class TestBuildPromptPairsSameLabels:
    ...
    def test_same_labels_true_filters_mismatched(self):
        ...
        from src.intertemporal.preference import build_prompt_pairs
        items = [

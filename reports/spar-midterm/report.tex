% SPAR Midterm Report — Temporal Awareness in LLMs
...
% ============================================================================
\begin{abstract}
We investigate how large language models internally represent and process temporal information, an open question with direct implications for AI safety --- particularly for understanding planning capability, deception detection, and oversight robustness. Across 12 complementary sub-projects, we apply linear probes, causal interventions (activation steering), and behavioral evaluations to models ranging from 1B to 30B parameters. Our preliminary findings show that temporal representations are genuinely encoded in LLM activations and can be causally steered, but that careful baselines are essential: in code generation, what appears to be planning signal is fully explained by surface features. We also find safety-relevant phenomena including context fatigue (entropy collapse over long contexts), rare but real sycophantic surrender under social pressure, and a link between short-term temporal steering and more harmful outputs. These results motivate both methodological caution in probing studies and further investigation of temporal representations as a lever for AI safety monitoring.
\end{abstract}
...
\textbf{Context fatigue.} Raj Roy demonstrates that entropy collapse is a real problem as context fills, with models growing more confident without corresponding accuracy gains. This is partially counteracted by in-context learning, where models sometimes improve at a task pattern over the context window.

\textbf{Uncertainty encoding.} Bandyopadhyay finds that uncertainty is strongly encoded at the first reasoning step but weakens after subsequent reasoning, is distributed across layers, and only weakly predicts future states. Across datasets, uncertainty encoding varies significantly and consistently degrades from step 1 to step 2.
...
Harleen Kaur Bagga & Sycophancy detection via hidden states & Sycophancy rare (1.5--15.5\%); confidence unreliable as monitor & Publication \\
...
\bottomrule
\end{tabular}
\caption{Summary of fellow research questions, key midterm findings, and target outputs.}
\label{tab:fellows}
\end{table}
...
\textbf{Statistical power.} Several sub-projects face sample size constraints. The sycophancy probe (Bagga) has only 11 positive examples for the critical hidden-state prediction task and needs approximately 1,000 to be conclusive. Scaling these experiments is a priority for the second half.
...
For the second half of the program, we prioritize the following:

\textbf{Scaling critical experiments.} Bagga will expand the sycophancy dataset to $\sim$1,000 positive examples to properly power the hidden-state prediction probe. Molofsky will complete Phase 2 (activation probes + SAEs) for the remaining 4 models, targeting precursor gap detection before accuracy degradation manifests.

\textbf{Testing generalization.} Paudel has 1,000 validated implicit temporal pairs across 40 categories that remain unused as a clean held-out test. Running probes and steering experiments on this set will establish whether findings generalize beyond training distributions. Sharma will extend cross-model analysis with causal patching to determine whether temporal representations are functionally equivalent across architectures.

\textbf{Confirming safety links.} Obiso's observation that short-term steering increases violent completions requires systematic confirmation with larger sample sizes and controlled generation. If confirmed, this would be a strong safety-relevant finding linking temporal representations to harmful behavior.

\textbf{New directions.} Gupta will test the phase transition hypothesis for temporal horizon representations. Goel will complete the token trajectory analysis for multihop reasoning. Raj Roy will further characterize the interaction between context fatigue and in-context learning.

\textbf{Target venues.} We aim for NeurIPS 2026 (main conference or workshops) for the strongest results, with blog posts and web demos for preliminary or negative findings. The baseline staircase methodology (Dahiya) and temporal steering results (Obiso, Paudel) are the most publication-ready.

...
from .simple_binary_choice import LabeledSimpleBinaryChoice, SimpleBinaryChoice
...
@dataclass
class GroupedBinaryChoice(BinaryChoice):
    """Binary choice aggregated from multiple forks (label pairs).

    Each fork is a LabeledSimpleBinaryChoice representing one label style.
    The overall choice is determined by aggregating across forks.

    Attributes:
        forks: List of binary choices, one per label pair.
        aggregation: Method for combining fork results.
    """

    forks: list[LabeledSimpleBinaryChoice] = field(default_factory=list)
    aggregation: ForkAggregation = ForkAggregation.MEAN_LOGPROB

    # ── Factory ──────────────────────────────────────────────────────────

    @classmethod
    def from_simple_choices(
        cls,
        choices: list[LabeledSimpleBinaryChoice],
        aggregation: ForkAggregation = ForkAggregation.MEAN_LOGPROB,
        **kwargs: Any,
    ) -> GroupedBinaryChoice:
        """Build from existing LabeledSimpleBinaryChoice objects."""
        return cls(forks=choices, aggregation=aggregation, **kwargs)
    ...
    def fork_choices(self) -> list[int]:
        ...
        return [f.choice_idx for f in self.forks]
    ...
    def disagreeing_forks(self) -> list[int]:
        ...
        if overall == -1:
            return []
        return [i for i, f in enumerate(self.forks) if f.choice_idx != overall]

    def fork_logprob_diffs(self) -> list[float]:
        """Logprob difference (A - B) for each fork."""
        diffs = []
        for f in self.forks:
            lp_a = f._divergent_logprobs[0]
            lp_b = f._divergent_logprobs[1]
            diffs.append(lp_a - lp_b)
        return diffs

    def divergent_positions(self) -> list[int | None]:
        """Divergent position for each fork."""
        return [f.divergent_position for f in self.forks]
    ...
    def _aggregated_logprobs_by_method(
        ...
        lps_a = [f._divergent_logprobs[0] for f in self.forks]
        lps_b = [f._divergent_logprobs[1] for f in self.forks]
    ...
    def _mean_normalized_logprobs(self) -> tuple[float, float]:
        ...
        for f in self.forks:
            lp_a, lp_b = f._divergent_logprobs
    ...
    def _vote_choice_idx(self) -> int:
        ...
        votes_a = sum(1 for f in self.forks if f.choice_idx == 0)
        votes_b = sum(1 for f in self.forks if f.choice_idx == 1)
        if votes_a > votes_b:
            return 0
        if votes_b > votes_a:
            return 1
...
class LabeledGroupedBinaryChoice(GroupedBinaryChoice, LabeledBinaryChoice):
    ...
    def from_simple_choices(
        ...
        **kwargs: Any,
        ...
        Extracts labels from the fork choices automatically.
        """
        # Extract labels from forks
        labels_a = []
        labels_b = []
        for choice in choices:
            if choice.labels:
                labels_a.append(choice.labels[0])
                labels_b.append(choice.labels[1])

        labels = (labels_a, labels_b) if labels_a else None

        return cls(forks=choices, aggregation=aggregation, labels=labels, **kwargs)

    # ── Labels Interface ─────────────────────────────────────────────────

    @property
    def chosen_label(self) -> str | None:
        """First label of the chosen option (for compatibility)."""
    ...
    def alternative_label(self) -> str | None:
        """First label of the alternative option (for compatibility)."""
        if self.labels is None:
            return None
        idx = self.choice_idx
        if idx == -1:
    ...
    def without_labels(self) -> GroupedBinaryChoice:
        ...
        # Also strip labels from each fork
        unlabeled_forks = [
            SimpleBinaryChoice(tree=f.tree) for f in self.forks
        ]
        # Note: This creates SimpleBinaryChoice, not LabeledSimpleBinaryChoice
        # Type is correct but we need to handle it in aggregation
        return GroupedBinaryChoice(
            forks=unlabeled_forks,  # type: ignore

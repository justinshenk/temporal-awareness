...
class ContrastivePair(BaseSchema):
    ...
    def available_layers(self) -> list[int]:
        ...
        layers = set()
        for hook in self.short_cache.keys():
            if hook.startswith("blocks.") and ".hook_" in hook:
                try:
                    layers.add(int(hook.split(".")[1]))
    ...
    def get_interventions(
        ...
        return [
            self._make_intervention(target, l, component, mode, alpha)
            for l in layers
            if self._get_acts(self.short_cache, l, component) is not None
        ]
    ...
    def _make_intervention(
        ...
        # Determine source/target based on mode
        if mode == "denoising":
        ...
        if alpha < 1.0:
            return Intervention(
                ...
                alpha=alpha,
                target=InterventionTarget.at_positions(positions)
                if positions
    ...
    def print_summary(self) -> None:
        print(
            f"Short: {self.short_length}, Long: {self.long_length}, Layers: {self.available_layers[:5]}..."
        )

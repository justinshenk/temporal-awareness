...
from ..experiments.coarse.coarse_viz import visualize_coarse_patching, visualize_component_comparison
from ..experiments.diffmeans.diffmeans_viz import visualize_diffmeans_pair
from .tokenization_viz import (
    TokenizationVizData,
    visualize_tokenization,
    visualize_tokenization_from_cache,
)
from ..experiments.fine import visualize_fine
...
if TYPE_CHECKING:
    ...
    from ..experiments.fine import FineResults
...
def visualize_pair_results(
    ...
    *,
    ...
    fine_result: "FineResults | None" = None,
    ...
    coarse activation patching, fine activation patching, diffmeans, and
    ...
    Args:
        ...
        fine_result: Fine-grained patching results (plots 17-26)
    ...
    # Plots are saved as {component}_{mode}.png directly in attrib/
    if att_result is not None:
        att_dir = pair_out_dir / "attrib"
    ...
    if diffmeans_result is not None and diffmeans_result.position_results:
        visualize_diffmeans_pair(diffmeans_result, pair_out_dir / "diffmeans", runner=runner, pair=pair)

    # Step 7: Fine-grained patching visualization
    if fine_result is not None:
        visualize_fine(fine_result, pair_out_dir / "fine", position_mapping)

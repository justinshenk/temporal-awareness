...
def _print_pair_details(pair: ContrastivePreferences, idx: int) -> None:
    ...
    log(f"  Pair {idx + 1}:")
    log(f"    Short chooser: sample {short.sample_idx}, horizon={_format_horizon(h_short)}, prob={_format_prob(short.choice_prob)}")
    log(f"    Long chooser:  sample {long.sample_idx}, horizon={_format_horizon(h_long)}, prob={_format_prob(long.choice_prob)}")
    log(f"    Rational: short={_format_rational(short.matches_rational)}, long={_format_rational(long.matches_rational)}")
    log(f"    Associated: short={_format_rational(short.matches_associated)}, long={_format_rational(long.matches_associated)}")
    ...
    log(f"    Same content: rewards={pair.same_rewards}, times={pair.same_times}, labels={pair.same_labels}")
...
def print_contrastive_pairs(pairs: list[ContrastivePreferences]) -> None:
    ...
    # For small numbers of pairs, show individual details
    if n <= 5:
        _section("PAIR DETAILS")
        log("")
    ...
    # Horizon pair breakdown (only show if more than 3 unique pairs)

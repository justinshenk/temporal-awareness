...
from ...common.semantic_positions import TIME_POSITIONS
...
class DiffMeansConfig(BaseSchema):
    ...
    positions: list[str] = field(default_factory=lambda: list(TIME_POSITIONS))

    # Visualization settings
    critical_layer_ranges: list[tuple[int, int]] = field(
        default_factory=lambda: [(19, 24), (28, 34)]
    )
    reference_layers: list[int] = field(default_factory=lambda: [19, 24, 31])

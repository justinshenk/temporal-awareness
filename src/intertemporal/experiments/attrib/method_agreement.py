...
class MethodAgreementResults(BaseSchema):
    ...
    def print_summary(self) -> None:
        ...
        print(
            f"Method Agreement ({self.mode}): {self.overall_agreement} (mean Jaccard={self.mean_jaccard:.3f})"
        )
        ...
        if self.pair_agreements:
            ...
            print(
                f"  Best pair: {top_agreement.method_a} vs {top_agreement.method_b} ({top_agreement.jaccard:.3f})"
            )
            print(
                f"  Worst pair: {low_agreement.method_a} vs {low_agreement.method_b} ({low_agreement.jaccard:.3f})"
            )

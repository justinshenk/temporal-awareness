...
from ...common.file_io import load_json, save_json, ensure_dir
...
class PreferenceDataset(BaseSchema):
    ...
    def filter_valid(self) -> list[PreferenceSample]:
        ...
        return [p for p in self.preferences if p.choice_term in ("short_term", "long_term")]

    def _to_dict(self, without_long_sequences: bool = False) -> dict:
        preferences_data = []
        for pref in self.preferences:
            max_list_length = 10 if without_long_sequences else None
            max_string_length = 20 if without_long_sequences else None
    ...
    def to_string(
        ...
        without_texts: bool = False,
        without_long_sequences: bool = False,
    ) -> str:
        d = self._to_dict(without_long_sequences=without_long_sequences)

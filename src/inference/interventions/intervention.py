...
class InterventionTarget(BaseSchema):
    ...
    @classmethod
    def all(cls) -> InterventionTarget:
        return cls(axis="all")

    @classmethod
    def at_positions(cls, positions: Union[int, list[int]]) -> InterventionTarget:
        if isinstance(positions, int):
            positions = [positions]
        return cls(axis="position", positions=positions)

    @classmethod
    def at_neurons(cls, neurons: Union[int, list[int]]) -> InterventionTarget:
        if isinstance(neurons, int):
            neurons = [neurons]
        return cls(axis="neuron", neurons=neurons)

    @classmethod
    def on_pattern(cls, pattern: str) -> InterventionTarget:
...
def load_intervention_from_dict(data: dict, n_layers: int) -> Intervention:
    ...
    Args:
        data: Dict with keys: layer, mode, values, target, component, strength
        n_layers: Number of layers in model (to clamp layer index)
    ...
    if target_data == "all" or target_data is None:
        ...
        axis = target_data.get("axis", "all")
        if axis == "position":
            target = InterventionTarget.at_positions(target_data["positions"])
        elif axis == "neuron":
            target = InterventionTarget.at_neurons(target_data["neurons"])
        elif axis == "pattern":
            target = InterventionTarget.on_pattern(target_data["pattern"])
        else:
            target = InterventionTarget.all()
    else:
        target = InterventionTarget.all()

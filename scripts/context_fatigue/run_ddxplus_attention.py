"""
DDXPlus Attention Analysis — Context Fatigue

Monkey-patches Qwen2 attention to only capture weights at target layers.
All other layers use sdpa (memory efficient). This avoids OOM from
materializing 28 full attention matrices.

For each case, records per-head at target layers:
  - Attention entropy
  - Where attention lands (system, early, recent, current query)
"""
...
def parse_args():
    ...
    p.add_argument("--layers", default="0,7,15,27")
    p.add_argument("--out-dir", default="results/ddxplus_attention")
...
# ── selective attention capture via manual QK computation ───────────────

class SelectiveAttentionCapture:
    """
    Hook into q_proj and k_proj outputs (post-RoPE) at target layers.
    Compute attention weights manually for just the last query token.
    This avoids materializing full NxN attention at non-target layers.
    """

    def __init__(self, model, target_layers):
        self.target_layers = set(target_layers)
        self.captured = {}  # layer -> (n_heads, seq_len) last-token attention
        self.hooks = []
        self.enabled = False  # only capture when explicitly enabled

        for li in target_layers:
            attn = model.model.layers[li].self_attn
            hook = attn.register_forward_pre_hook(self._make_attn_hook(li), with_kwargs=True)
    ...
    def _make_attn_hook(self, layer_idx):
        def hook_fn(module, args, kwargs):
            ...
            if hidden_states is None or position_embeddings is None:
                return
            # Skip during cached generation (seq_len=1)
            ...
            with torch.no_grad():
                ...
                # Only compute attention for last query token
                q_last = q[:, :, -1:, :]
...
def main():
    ...
    for case_num, idx in enumerate(valid_indices):
        ...
        # Generate response (hooks disabled)
        with torch.no_grad():

...
from scripts.attribution.attribution_common import gsm8k_problems, load_base_and_lora, load_contrast, prompt_token_ids
...
def eval_band(base, lora, tok, problems, device, L, Vband, max_new):
...
def main() -> None:
    ...
    pca_problems = gsm8k_problems("train", args.n_pca, skip=0)
    print(f"Collecting δ at L{L} over {len(pca_problems)} problems for PCA ...", flush=True)
    _, D = collect_base_traj(base, lora, tok, pca_problems, device, L, args.max_new)
    print(f"  PCA over {D.shape[0]} tokens ...", flush=True)
    V, lam = pca_bands(D)
    V = V.to(torch.float32)

    contrast = load_contrast(cfg)[:args.n_contrast]
    results = {"layer": L, "n_pca_tokens": int(D.shape[0]), "n_contrast": len(contrast),
               "max_new": args.max_new, "bands": {}}
    ...
    for spec in bands:
        ...
        acc = eval_band(base, lora, tok, contrast, device, L, Vb, args.max_new)
        results["bands"][spec] = {"k": int(k), "energy_frac": float(ef), "recovery": acc}
        print(f"{spec:10s}  {ef:6.3f}        {acc:.3f}", flush=True)

    out_path = Path(args.out) if args.out else Path(cfg["output"]["steer_json"]).parent / f"lockstep_pca_band_L{L}.json"

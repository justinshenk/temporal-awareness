...
|---|--:|--:|
| ridge steer @L20 | **+0.26** scan / **+0.35** contrast (steer_acc 0.165 on the 200-scan; 0.350 on 20 contrast) | ≈0.05 |
| nonlinear MLP @L20 | **+0.00** (val cos +0.822 / R² +0.675 vs ridge +0.636 / +0.270 — better fit, zero closed-loop) | 0.00 (same paradox) |
...
pointwise map by ≥0.4 of the budget — but it is *lower* for multihop: hop composition over given
...
| pointwise ladder | ≈0 (ridge/MLP/DAgger/DAS) | MLP +0.00, DAgger +0.00 — but **ridge +0.26–0.35** | **PARTIAL** |
...
wall exists (oracle beats every map by ≥0.4 of the budget; better-fitting nonlinear and on-policy
estimators still collapse to 0) but the *linear* rung leaks ~⅓ of the budget on multihop vs ~5% on
arithmetic. Reading: hop composition over passages given in-context has a larger register-like
(linearly transportable) component than arithmetic — consistent with the thesis's own
register-vs-procedure split rather than against it, but it sharpens the claim: "procedures do not
install" should be "the *procedure core* does not install; its size is task-dependent."

Honest caveats: n = 2 procedures; ridge divergence measured at one layer (L20) and one α (1.0);

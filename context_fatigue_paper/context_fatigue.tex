...
\begin{table}[t]
  ...
  \texttt{code} cell). Full-transcript row $n{=}39$, bisection rows $n{=}23$. $\Delta\Delta$
  $=$ shift of the instructed-formats
  log-probability contrast, against a self-patch baseline (Appendix~\ref{app:qwen}).}
  \label{tab:patch}
  \setlength{\tabcolsep}{5pt}
  \begin{tabular}{lcc}
    \toprule
...
difference-in-differences $\Delta\Delta$ against a \emph{self-patch} baseline, the
recipient's own open-captured states pushed through the identical procedure, not against the
unpatched transcripts. Donor states are captured with the system span open while a pure run's

...
\textbf{We withdraw a positive result from an earlier version of this work.} That version reported
a late-window accuracy dip on the random-subject stream at ${\geq}80\%$ fill
($-0.141$ $[-0.249,-0.031]$, $n{=}91$), read as accumulation finally outrunning in-context
learning. Re-running the protocol, our driver turned out to present the \emph{identical} questions
as the committed run (per-item agreement $1.000$ over 344 items, accuracy $0.6221$ both), so the
harness is bit-identical and any disagreement is sample composition. Finer bins locate the effect:
accuracy is $0.625$ at $0.80$--$0.85$ fill ($n{=}40$), $\mathbf{0.419}$ at $0.85$--$0.88$
($n{=}31$), and $0.703$ at $0.88$--$0.93$ ($n{=}37$). The original run's maximum fill was $0.8784$,
so its entire top bin \emph{is} that one trough. Extending the same sessions to $93\%$ gives
$-0.097$ (n.s.); 14 fresh sessions give $+0.090$ (opposite sign); all 26 pooled give
$+0.005$ $[-0.105,+0.092]$. Overflow-guard selection ($1/781$ skipped) and item length are ruled
out. A clamp-based rescue arm was null but too weak an instrument to carry weight. The explanandum
does not exist, and the account here no longer needs it.
Beyond that: an earlier analysis of these same runs resampled the arms independently rather than
pairing over shared items, inflating each interval about $2.5\times$ and reporting one real effect
as null. The necessity direction of the mediation is only partially established and is plausibly
instrument-limited, since the clamp is uniform across heads. The competition result is one

...
\emph{We also withdraw a positive result from an earlier version of this work}
(\S\ref{sec:withdrawn}).
\end{abstract}

\section{Accumulation and dilution are confounded everywhere else}
...
This abstract separates them inside one harness, and finds that \textbf{accumulation is free and
dilution is not}---then measures the dilution cost causally, in units of attention mass.

\section{Design: localized tasks make accumulation and dilution independent}
...
All causal experiments below run on OLMo-2-7B-Instruct at layer 24, seed 42, with an overflow
...
\section{Accumulation alone costs nothing}
...
\section{Move the evidence and the cost appears}
...
byte-identical, and the overflow guard skipped $0$ of $192$ probes.
...
Every gap's CI excludes zero (Table~\ref{tab:distance}, Fig.~\ref{fig:dilution}a). In a joint fit of accuracy on fill and
...
So the same model, on the same items at the same fill, degrades when the evidence is displaced and
does not degrade when it is not. The localized null is not a ceiling artifact: these items
\emph{can} show degradation, and do, as soon as dilution is reintroduced.

\section{The cost is causally mediated by attention mass}
...
We add a clamp that \emph{sets} a token span's post-softmax attention share: a constant bias on the
span's key columns scales its odds, and softmax renormalizes, so nothing in the attention forward
is reimplemented and a scale of $1.0$ is bit-identical to the unhooked model. Keeping the evidence
at \texttt{local} and clamping its share down to the \emph{same item's own} \texttt{back\_20}
share costs $+0.202$ $[+0.138,+0.267]$ (paired $n{=}174$) and lands at $0.333$ against
\texttt{back\_20}'s $0.365$---a difference of $-0.025$ $[-0.083,+0.035]$, indistinguishable.
...
clamp applies a \emph{uniform} bias across heads; it can strip mass faithfully but cannot
reconstruct \emph{which} heads should carry the restored mass. Full sufficiency with partial
necessity is the signature of an instrument that is faithful in one direction and crude in the
other, and we report it as one rather than as a second mechanism. A pattern-matched, per-head
clamp is the test that would settle it.
...
\section{Competition, separated from distance}

Burying a needle varies two things at once: the evidence becomes distant \emph{and} it becomes
surrounded by plausible alternatives. \S4--\S5 isolated distance. We isolate the other half by
...
one). Paired over $n{=}365$ probes:

\begin{center}\small
\begin{tabular}{lccc}
\toprule
arm & \texttt{random} (0.80) & \texttt{disjoint} (0.00) & \texttt{near\_dup} (3.75)\\
\midrule
accuracy & 0.512 & 0.485 & \textbf{0.427}\\
\bottomrule
\end{tabular}
\end{center}
...
variable. Two controls matter: the low-competition arms must land on \S4's \texttt{local} accuracy
...
$[-0.00088,+0.00035]$ in evidence share at layer 24. The dose-response of \S5 is $6.29$ accuracy
...
\section{A withdrawn result}
\label{sec:withdrawn}

An earlier version of this work reported one positive finding: a late-window accuracy dip on the
random-subject stream at ${\geq}80\%$ fill ($-0.141$ $[-0.249,-0.031]$, $n{=}91$), read as
accumulation finally outrunning in-context learning. \textbf{We withdraw it.} Re-running the
protocol, our driver turned out to present the \emph{identical} questions as the committed run
(per-item agreement $1.000$ over 344 items, accuracy $0.6221$ both), so the harness is
bit-identical and any disagreement is sample composition. Finer bins locate the effect: accuracy is
$0.625$ at $0.80$--$0.85$ fill ($n{=}40$), $\mathbf{0.419}$ at $0.85$--$0.88$ ($n{=}31$), and
$0.703$ at $0.88$--$0.93$ ($n{=}37$). The original run's maximum fill was $0.8784$, so its entire
top bin \emph{is} that one trough. Extending the same sessions to $93\%$ gives $-0.097$ (n.s.);
14 fresh sessions give $+0.090$ (opposite sign); all 26 pooled give $+0.005$ $[-0.105,+0.092]$.
Overflow-guard selection ($1/781$ skipped) and item length are ruled out. A clamp-based rescue arm
was null but too weak an instrument to carry weight. The explanandum does not exist, and the
paper's account no longer needs it: the end-of-window cost we thought we had is a single-bin
artifact, while the dilution cost of \S4--\S5 is large, causal and replicated.

\section{The confidence signatures are in-context comfort}
...
\section{What context rot actually is}
...
equally on \emph{wrong} answers ($0.86\to0.95$; $r=+0.69$ $[+0.57,+0.78]$)---so the confidently-wrong gap widens by ${\sim}10$ points over a session
(Fig.~\ref{fig:signatures}c), most consequential exactly where
a homogeneous high-stakes stream makes the model surest.

\paragraph{Limitations.}
Per-condition $n$ is bounded by the context window, so every clamp experiment is paired over
shared items and analysed with a paired bootstrap; an earlier analysis of these same runs
resampled the arms independently, inflating each interval about $2.5\times$ and reporting one real
effect as null. The necessity direction of the mediation is only partially established and is
plausibly instrument-limited. The clamp is uniform across heads. The competition result is one significant
step rather than a gradient---mild overlap is indistinguishable from none, and we do not claim
cost rises with confusability. The instruction-canary null is a
floor effect, the WildChat figures are summary values from teacher-forced analyses, and the causal
program runs on one model family. The accuracy null holds for localized tasks; genuinely
length-dependent single-task settings are where dilution is the point, and are out of scope by
design.

...
observed effect). Per head, that null is not inaction but cancellation: the evidence's mass is
reallocated across heads ($19$ of $32$ heads move at Bonferroni) with the total held fixed, a
signature displacement does not have---there, all $32$ heads lose mass at once. So context rot is two mechanisms, not one---displacement acts through attention mass and
...
manipulations rather than intervened on. This paper separates the three factors inside one harness
...
evidence's total attention mass}: \texttt{random} $-$ \texttt{near\_dup} $=-0.00027$
...
\emph{accuracy} difference and no share difference. ``Attention dilution'' is therefore the right
account of the first and the wrong account of the second---and a needle-in-a-haystack benchmark
varies both at once, which is precisely why they have not been told apart before.

\paragraph{Per head, the two mechanisms look nothing alike.}
The share above is a mean over the layer's $32$ heads, and a mean can hold still while heads
cancel. Re-measuring the same probes per head separates the two readings. Under displacement,
\textbf{all $32$ heads lose evidence mass} from \texttt{local} to \texttt{back\_20}, every one
significant at Bonferroni, so nothing is hidden by averaging (mean $|\Delta|$ equals $|$mean
$\Delta|$ exactly); the heads holding the most mass lose the most ($r{=}0.96$), though the
\emph{fractional} loss ranges from $32\%$ to $99\%$. Under competition the total is fixed but the
heads are not: for \texttt{random} $-$ \texttt{near\_dup} the mean per-head $|\Delta|$ is
$0.00257$ against a paired sign-flip null of $0.00043$ ($p\leq0.0005$, 2{,}000 permutations),
$\mathbf{9.6\times}$ the net change, with $19$ of $32$ heads individually significant at
Bonferroni. \textbf{Competition does not leave attention alone; it reallocates the evidence's mass
across heads at constant total.} That leaves the dilution arithmetic above intact---the
dose-response is denominated in the head-averaged share, and that is the quantity which did not
move---but it replaces ``no attention signature'' with a \emph{different} one. Whether the
reallocation causes the errors or follows from them needs a per-head clamp, which we have not run.
The evidence's mass is spread broadly in the first place (effective head count $27.3$ of $32$ at
\texttt{local}), so none of this is a two-head circuit.
